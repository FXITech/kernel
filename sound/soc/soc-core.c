...
found:
	...
	dev_dbg(dev, "Unregistered DAI '%s'\n", dai->name);
	kfree(dai->name);
	kfree(dai);
}
		...
		struct snd_soc_platform_driver *platform_drv)
{
	...
	dev_dbg(dev, "Registered platform '%s'\n", platform->name);
...
found:
	...
	dev_dbg(dev, "Unregistered platform '%s'\n", platform->name);
	kfree(platform->name);
	kfree(platform);
}
			   ...
			   int num_dai)
{
	...
	dev_dbg(dev, "Registered codec '%s'\n", codec->name);
	return 0;
...
found:
	...
	dev_dbg(dev, "Unregistered codec '%s'\n", codec->name);

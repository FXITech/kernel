...
static inline void s3c_udc_pre_setup(void)
{
	...
	__raw_writel((1 << 19) | sizeof(struct usb_ctrlrequest),
...
static void complete_rx(struct s3c_udc *dev, u8 ep_num)
{
	...
	__dma_single_cpu_to_dev(req->req.buf, req->req.length,
				DMA_FROM_DEVICE);
	...
	DEBUG_OUT_EP("%s: RX DMA done : ep = %d, rx bytes = %d/%d, "
			"is_short = %d, DOEPTSIZ = 0x%x, "
			"remained bytes = %d\n", __func__, ep_num,
			req->req.actual, req->req.length, is_short,
			ep_tsr, xfer_size);
...
static void complete_tx(struct s3c_udc *dev, u8 ep_num)
{
	...
	u32 ep_tsr = 0, xfer_size = 0, xfer_length;
	u32 last;
	...
	DEBUG_IN_EP("%s: TX DMA done : ep = %d, tx bytes = %d/%d, "
			"is_short = %d, DIEPTSIZ = 0x%x, "
			"remained bytes = %d\n", __func__, ep_num,
			req->req.actual, req->req.length,
			xfer_length < ep->ep.maxpacket, ep_tsr, xfer_size);
...
static void process_ep_in_intr(struct s3c_udc *dev)
{
	...
	while (ep_intr) {
		if (ep_intr & 0x1) {
			ep_intr_status =
				__raw_readl(S3C_UDC_OTG_DIEPINT(ep_num));
...
static void process_ep_out_intr(struct s3c_udc *dev)
{
	...
	while (ep_intr) {
		if (ep_intr & 0x1) {
			ep_intr_status =
				__raw_readl(S3C_UDC_OTG_DOEPINT(ep_num));
...
static irqreturn_t s3c_udc_irq(int irq, void *_dev)
{
	...
	DEBUG_ISR("\n*** %s : GINTSTS=0x%x(on state %s), GINTMSK :"
			...
			__raw_readl(S3C_UDC_OTG_DAINT),
			__raw_readl(S3C_UDC_OTG_DAINTMSK));
	...
	if (list_empty(&ep->queue) && !ep->stopped) {
		...
		if (ep_num == 0) {
			...
			DEBUG_IN_EP("%s: ep_is_in, S3C_UDC_OTG_GINTSTS=0x%x\n",
				__func__, __raw_readl(S3C_UDC_OTG_GINTSTS));
...
static int write_fifo_ep0(struct s3c_ep *ep, struct s3c_request *req)
{
	...
	DEBUG_EP0("%s: wrote %s %d bytes%s %d left %p\n", __func__,
			ep->ep.name, count,
			is_last ? "/L" : "", req->req.length - req->req.actual,
			req);
...
static inline void s3c_udc_ep0_set_stall(struct s3c_ep *ep)
{
	...
	DEBUG_EP0("%s: set ep%d stall, DIEPCTL0 = 0x%x\n",
			...
			__raw_readl(S3C_UDC_OTG_DIEPCTL(EP0_CON)));
	/*
...
void s3c_udc_ep_activate(struct s3c_ep *ep)
{
	...
	__raw_writel(__raw_readl(S3C_UDC_OTG_DAINTMSK) | daintmsk,
...
static inline void set_test_mode(void)
{
	...
	case TEST_PACKET_SEL:
		/* some delay is necessary like printk() or udelay() */
		printk(KERN_INFO "Test mode selector in set_feature request"
				"is TEST PACKET\n");

		__dma_single_cpu_to_dev(test_pkt, TEST_PKT_SIZE,
				DMA_TO_DEVICE);
		__raw_writel(virt_to_phys(test_pkt),
				S3C_UDC_OTG_DIEPDMA(EP0_CON));

		ep_ctrl = __raw_readl(S3C_UDC_OTG_DIEPCTL(EP0_CON));

		__raw_writel(1 << 19 | TEST_PKT_SIZE << 0,
				S3C_UDC_OTG_DIEPTSIZ(EP0_CON));
		__raw_writel(ep_ctrl | DEPCTL_EPENA | DEPCTL_CNAK
				| EP0_CON<<DEPCTL_NEXT_EP_BIT,
				S3C_UDC_OTG_DIEPCTL(EP0_CON));
...
static void s3c_ep0_setup(struct s3c_udc *dev)
{
	...
	dev->req_std =
		(usb_ctrl.bRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD;
	dev->req_config = 0;
	dev->req_pending = 1;
	...
	if (likely(dev->driver)) {
		...
		if (i < 0) {
			if (dev->req_config) {
				DEBUG_SETUP("\tconfig change 0x%02x "
					"fail %d?\n", (u32)&usb_ctrl.bRequest,
					i);
				return;
			}
